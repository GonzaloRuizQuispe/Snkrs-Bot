...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.online, activity=discord.Game("Discord Manager"))
...
async def seccion(ctx,web,url):
    ...
    if connection.is_connected():
        ...
        cursor.execute("SELECT URL_WEB FROM secciones_"+web+" WHERE URL_WEB='"+url[1:-1]+"'")
        cantidad_f = cursor.fetchone()

        if cantidad_f == None:
            cursor.execute("INSERT INTO secciones_"+web+" (URL_WEB) VALUES ('"+url[1:-1]+"')")
            connection.commit()
            await ctx.message.delete()
